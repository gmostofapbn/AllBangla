...
export default async function HomePage() {
  const [sections, homePosts, globalOpenExternal, outletTotal, settings] =
    await Promise.all([
      getHomeSections(),
      getHomePosts(3),
      getDefaultOpenExternal(),
  ...
  const faqs = homeFaqs(outletTotal);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-12 px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
      <JsonLd
        data={jsonLdGraph(
          collectionSchema({
            name: `${siteName} - every Bangladeshi newspaper in one place`,
      ...
      <HomeIntro siteName={siteName} outletCount={outletTotal} />
